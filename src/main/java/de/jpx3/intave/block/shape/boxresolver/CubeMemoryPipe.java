...
public final class CubeMemoryPipe implements ResolverPipeline {
  ...
  public List<BoundingBox> resolve(World world, Player player, Material type, int blockState, int posX, int posY, int posZ) {
    if (solidMaterials.contains(type)) {
      ...
      return Collections.singletonList(new BoundingBox(posX, posY, posZ, posX + 1, posY + 1, posZ + 1));
    } else if (otherMaterials.contains(type)) {
      return forward.resolve(world, player, type, blockState, posX, posY, posZ);
    }
    List<BoundingBox> resolve = forward.resolve(world, player, type, blockState, posX, posY, posZ);

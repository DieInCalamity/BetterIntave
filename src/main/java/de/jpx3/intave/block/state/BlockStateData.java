...
public abstract class BlockStateData<T> {
  ...
  public static final class BlockStateServerBridge {
    ...
    public static <T> T valueOf(User user, Block block, BlockStateData<T> blockStateData) {
      return INTERFACE_RESOLVE ? invokeSpecialResolve(user, block, blockStateData) : invokeInterfaceResolve(block, blockStateData);
    }

    @PatchyAutoTranslation
    private static <T> T invokeSpecialResolve(User user, Block block, BlockStateData<T> blockStateData) {
